...
import { graphicsMap } from './graphics';
...
function CollarSweater({ color }: { color: string }) {
  const path1 = useId();
  const mask1 = useId();
  return (
    <g
      ...
      <use
        ...
        xlinkHref={'#' + path1}
      />
      <g mask={`url(#${mask1})`} fillRule='evenodd' fill={color}>
        <rect x='0' y='0' width='264' height='110' />
      </g>
      <path
...
function GraphicShirt({ color, graphicType }: { color: string; graphicType: string }) {
  const path1 = useId();
  const mask1 = useId();
  const GraphicComponent = graphicsMap[graphicType as keyof typeof graphicsMap];
  return (
    <g
      ...
      {GraphicComponent && <GraphicComponent maskID={mask1} />}
    </g>
  );
}
...
function Overall({ color }: { color: string }) {
  const path1 = useId();
  const mask1 = useId();
  return (
...
function ShirtCrewNeck({ color }: { color: string }) {
  const path1 = useId();
  const mask1 = useId();
  return (
    <g
...
function ShirtScoopNeck({ color }: { color: string }) {
  const path1 = useId();
  const mask1 = useId();
  return (
    <g
...
function ShirtVNeck({ color }: { color: string }) {
  const path1 = useId();
  const mask1 = useId();
  return (
...
export const clothesMap: Record<ClotheType, (props: { color: string; graphicType?: string }) => React.JSX.Element> = {
  BlazerShirt: () => <BlazerShirt />,
  BlazerSweater: () => <BlazerSweater />,
  CollarSweater: ({ color }) => <CollarSweater color={color} />,
  GraphicShirt: ({ color, graphicType }) => <GraphicShirt color={color} graphicType={graphicType || 'Bat'} />,
  Hoodie: ({ color }) => <Hoodie color={color} />,
  Overall: ({ color }) => <Overall color={color} />,
  ShirtCrewNeck: ({ color }) => <ShirtCrewNeck color={color} />,
  ShirtScoopNeck: ({ color }) => <ShirtScoopNeck color={color} />,
  ShirtVNeck: ({ color }) => <ShirtVNeck color={color} />,
};
